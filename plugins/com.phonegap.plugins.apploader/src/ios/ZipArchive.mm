...
@synthesize zipFilename, unzipFilename;
...
-(uLong) UnzipGetNumberOfEntries
{
	...
	if( _unzFile )
	{
		...
		{
			entriesCount = globalInfo.number_entry;
		}
	}
	return entriesCount;
}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		[self fileUnzippedFromZipfile:self.unzipFilename filename:[NSString stringWithCString:filename encoding:NSUTF8StringEncoding] 
						   entryTotal:globalInfo.number_entry entryNumber:++ctr 
						   compressed:fileInfo.compressed_size uncompressed:fileInfo.uncompressed_size];
...
- (void) fileUnzippedFromZipfile:(NSString*)zipFile filename:(NSString*)filename entryTotal:(uLong)entryTotal entryNumber:(uLong)entryNumber 
					  compressed:(uLong)compressedBytes uncompressed:(uLong)uncompressedBytes;
{
	if( _delegate && [_delegate respondsToSelector:@selector(fileUnzippedFromZipfile:filename:entryTotal:entryNumber:compressed:uncompressed:)] )
		return [_delegate fileUnzippedFromZipfile:zipFile filename:filename entryTotal:entryTotal entryNumber:entryNumber 
