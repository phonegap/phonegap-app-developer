...
API вызывает функцию обратного вызова успех с интервалом, просила, но ограничивает спектр запросов устройства между 40ms и 1000 мс. Например если вы запрашиваете интервалом в 3 секунды, (3000ms), API запрашивает данные от устройства каждую 1 секунду, но выполняет только успех обратного вызова каждые 3 секунды.
...
Содержит `Accelerometer` данные на определенный момент времени. Ускорение значения включают эффект гравитации (9,81 м/с ^ 2), так что когда устройство лежит плоская и вверх, *x*, *y*, и *z* значения, возвращаемые должны быть `` , `` , и`9.81`.
